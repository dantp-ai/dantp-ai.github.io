// Renderer for the minimal "blueprint" instrument: the grid arena (intent heatmap
// = per-cell predicted value, predicted-next cell, last-clicked context, reward
// bursts, slick ink cursor), the value-trace scope, and the radial anticipation
// gauge. Drawing is a pure function of the snapshot the main loop hands in; only
// short-lived effects (bursts) are owned here. Colors follow CSS custom properties
// so the canvas tracks light/dark theme.
...
export function createRenderer({ arena, scope, gaugeArc, gaugeValue }) {
  ...
  let INK = "#16181d";
  let BLUE = "#2b39e0";
  let CELLSEP = "rgba(20, 23, 28, 0.10)";

  function readTheme() {
    const s = getComputedStyle(document.documentElement);
    INK = s.getPropertyValue("--ink").trim();
    BLUE = s.getPropertyValue("--blue").trim();
    CELLSEP = rgba(INK, 0.10);
  }
  readTheme();
  ...
  return { resize, renderArena, renderScope, setGauge, addBurst, refreshTheme };
}
